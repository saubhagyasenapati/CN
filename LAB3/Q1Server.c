// Write a C program to create a TCP socket through which client will
// sent a string to the server and server will echoback the string to the 
// client.When client will sent exit then server will drop the connection.
/*
...
while(flag)
{
			...
			//printf("You sent '%s'\n", buf);
            // char message[50] = str;
            // strcpy(buf, strcat("You sent ", buf));
			send(fd1, buf, 100, 0);
}
